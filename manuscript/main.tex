...
nominal parameters ($h=40$\,\textmu m, $|\Phi^{(1)}|\sim100$\,\textmu V,
feature scale $\sim30$\,\textmu m) this gives
$|\Delta\mathbf{r}|\approx1.3\times10^{-6}$\,m, i.e.\ $\approx4\times10^{-2}$
of the feature scale, and the resulting frequency correction is
$\mathcal{O}(|\Delta\mathbf{r}|^2/\lambda_{\rm feature}^2)\sim
2\times10^{-3}$ relative --- a bounded Type-B systematic with known
analytic form, evaluated at the shifted equilibrium whenever the
...
residual charge density after UV cleaning, obtained from published
surface-potential maps~\cite{ziomba2025uv} through the
Dirichlet-to-Neumann map of Eq.~(\ref{eq:patch_mapping}): a residual
patch potential of $3.2$\,mV at the budget wavenumber
...
projected quantum-limited detection sensitivity; none is free of the
electrostatic identifiability limit either.
...
simulated ion-signal maps and reference reconstructions), is available in a
GitHub repository (\url{https://github.com/dawidkucharski/qespm}) with a
permanent Zenodo deposit; the repository is available to the editor and
reviewers upon request during peer review and will be made publicly
accessible at acceptance at the latest.
