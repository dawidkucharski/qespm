...
$r_{\rm eff} \propto 1/h$ on the analysed 1D cross-section.  A theorem establishes the topography--charge
...
persistent and growing need exists for non-contact surface
characterisation at micrometre standoff in ultra-high vacuum (UHV) and at
cryogenic temperatures, driven by the quantum-device community:
superconducting qubit electrode
...
amorphous dielectrics~\cite{muller2019quantum}.  Although AFM and STM
already operate in situ in UHV and at cryogenic temperatures, they
...
realised to date~\cite{keller2015electrometry}.  A single ion confined in a radio-frequency (rf) Paul trap
...
community has invested two decades in characterising and eliminating
...
deliberately using an ion to probe a surface was pioneered by Maiwald
...
surface topography $z(x,y)$ from ion motional observables with a defined
instrument transfer function, regularised inversion and an uncertainty
budget --- has not, to the present knowledge, been formulated.
The ion has been used as a qualitative electrometer --- detecting the
presence of surface fields --- but never as a quantitative
...
single-spin level~\cite{dolde2011nv}; the extension of electric-field
quantum sensing to quantitative surface topography reconstruction with
explicit instrument characterisation has likewise not, to the present
knowledge, been presented.

This work addresses this gap.  The present contribution is a
proposed theoretical framework: analytically derived, numerically
...
influence quantity that is structurally confounded with the
measurand (Sec.~\ref{sec:degeneracy}).  The complete quantity hierarchy
is summarised in Table~\ref{tab:quantities}:
...
\textit{Proof sketch.}  Each normal derivative acts as Fourier
multiplication by at most $k_{\max}^{n+1-m}$, giving the majorant sequence
$a_{n+1}=\sum_{m=1}^{n}(z_{\max}k_{\max})^{n+1-m}a_m/(n+1-m)!$ with
generating function $A(x)=a_1x/(2-e^{z_{\max}k_{\max}x})$ and convergence
radius $(\ln 2)/(z_{\max}k_{\max})$ --- consistent with, and weaker than,
the sharp Rayleigh criterion $kA\lesssim0.448$ of
Lemma~\ref{lem:perturbation} (full derivation: Supplementary Material,
Sec.~S10).
...
The surface field gradient displaces the ion equilibrium by
$\Delta\mathbf{r} = -(e/m\omega_{\rm sec}^2)\nabla\Phi^{(1)}$.  The
leading correction to the frequency shift is linear in the
displacement: evaluating Eq.~(\ref{eq:dw}) at the shifted equilibrium
gives
$\Delta\omega_x(\mathbf{r}_0+\Delta\mathbf{r}) =
\Delta\omega_x(\mathbf{r}_0)\,[1+\mathcal{O}(k|\Delta\mathbf{r}|)]$,
where $k$ is the surface wavenumber.  For the nominal grating
($A=100$\,nm, $\lambda=30$\,\textmu m, $h=40$\,\textmu m)
$k|\Delta\mathbf{r}|\approx6\times10^{-4}$, while for the strongest
test signal of this work (the Gaussian plateau,
$|\Phi^{(1)}|\sim100$\,\textmu V over a feature scale
$\sim15$\,\textmu m) it reaches $\mathcal{O}(10^{-1})$.  The correction
has a known analytic form ($\Delta\mathbf{r}\propto\nabla\Phi^{(1)}$),
so it can be applied as a bounded Type-B systematic wherever the
$0.1\%$ level matters.  Throughout this work
Eq.~(\ref{eq:dw}) is evaluated at the unperturbed equilibrium
$\mathbf{r}_0 = (0,0,h)$: for the nominal grating this approximation is
accurate to ${\sim}10^{-3}$ relative, while for the plateau test case
the equilibrium-shift correction is of order $10^{-1}$ and must be
included whenever that test case is used for quantitative conclusions.
...
field at the ion (the self-image potential) acquires its own
...
potential, Hankel-transformed and Poisson-propagated to the ion, yields
Eq.~(\ref{eq:phi_selfimage}); the adiabatic limit
$\tilde{\Phi}^{(1)}_{\rm im}\to-(e/8\pi\varepsilon_0h^2)\tilde{z}_s$ as
$kh\to0$ follows.  The identity was verified against direct quadrature of
the Poisson integral to $10^{-15}$ relative accuracy and against the
spectral chain to within $3\%$ across $kh\in[0.5,8]$ (full derivation:
Supplementary Material, Sec.~S10).  The complete instrument transfer function is therefore
\begin{equation}
...
matches each expansion at its own end, is not uniformly valid:
...
$x^2e^{-x}=2e^{-2}$.  The ITF is a deliberately broad band-pass
...
$\mathcal{H}_h$ is a bounded Fourier multiplier on $L^2(\mathbb{R}^2)$
whose spectrum is $\sigma(\mathcal{H}_h) = \overline{\operatorname{ran}H_x}
= [-M,0]$ with
...
A translation-invariant operator with non-vanishing symbol is not
compact on the infinite plane; the exponential ill-conditioning follows
instead from the inverse multiplier $H_x^{-1}\propto e^{+kh}/k_x^2$,
which amplifies high-$k$ errors exponentially.  Compactness --- and the
discrete singular spectrum analysed below --- are recovered on the finite
periodic lattices used for all numerical work.
...
heating channel is a functional of the fluctuation spectrum
...
sideband.  Two laser-noise channels enter: (i)~laser frequency
...
(ii)~intensity noise produces a differential ac-Stark shift
...
$\alpha=3.7$ --- a representative anchor adopted from the published
$3.5$--$4$ scaling range for surface-electrode traps, as reviewed by
Brownnutt et al.~\cite{brownnutt2015review}; the heating-limited optimum
$h_{\rm opt}=\alpha/k$ is independent of the anchor magnitude --- caps the
...
heating dominates and the achievable frequency resolution degrades as $h^{-4}$ (Fig.~\ref{fig:noise_budget}).  In this work SNR
values are quoted using $\delta\omega/2\pi = 5$\,Hz unless otherwise
noted.  This floor is a stated engineering assumption, not a measured
value: it deliberately exceeds by orders of magnitude the stability of the
frequency standards themselves (GPS-disciplined oscillators) and absorbs
residual technical drifts (rf amplitude noise, uncompensated micromotion,
ambient effects).  All SNR-dependent results scale linearly with
$\delta\omega$ ($A_{\min}\propto\delta\omega$), while the
identifiability-limited budget of Sec.~\ref{sec:uncertainty} is
independent of it, and the $h$-dependence is discussed explicitly.
...
QESPM observable is the field-gradient noise
$\partial E_x/\partial x$, not the field noise itself, so the spectral
density must be derived for the gradient.  In the quasistatic limit ($\omega \ll c/d$, skin depth
$\delta_s \gg d$), the fluctuation--dissipation theorem in the interfacial
Green's-function formulation of Wylie and Sipe~\cite{wylie1984quantum}
(applied to trapped ions by Henkel et al.~\cite{henkel1999loss}) gives
the field spectral density of a half-space of resistivity $\rho$
\begin{equation}
S_E(\omega;d) = \frac{k_B T\rho}{4\pi d^3},
\label{eq:johnson_field}
\end{equation}
in agreement with the FDT result for the Ohmic image drag on a charge
oscillating normally to the surface (full derivation: Supplementary
Material, Sec.~S15).  The gradient observable is the $k_x^2$-weighted
integral,
$S_{\partial E_x/\partial x}(\omega;d) = 3k_B T\rho/(8\pi d^5)$, and the
resulting frequency-shift noise, for a measurement of bandwidth
$1/\tau$,
\begin{equation}
...
$8\times10^{-6}$\,Hz --- six to eight orders of magnitude below the
technical noise floor across the operating envelope.  The validity
...
half-width $0.51\,h$, so the heating channel has the same lateral
localisation scale $\sim h$ as the electrostatic channel.  Under the fluctuating-patch
model $S_{\rm patch}\propto|\sigma(\mathbf{r})|^2$ (charge-variance
weighted), the heating channel is a quadratic functional of the
charge field: it is not annihilated by the linear rank-1 degeneracy of
Theorem~\ref{thm:degeneracy}, and it therefore provides information on
the charge-variance field outside the linear degeneracy.  Whether that
information restores joint identifiability in a practically invertible
sense is an open question requiring the full nonlinear joint analysis,
which is deferred to the Discussion
...
\begin{enumerate}
    ...
    \item \textbf{Uniform background field}: $E_{\rm bg}$ is constant across
          ...
          pseudopotential gradient varies slowly near the trap centre; its
          spatial structure is characterised in calibration step (1) of the
          Supplementary Material (Sec.~S7), and the residual variation is
          absorbed into the calibration uncertainty.
    \item \textbf{Perfect conductor}: The surface is an ideal grounded
          ...
          \end{equation}
          i.e.~a slight enhancement (the effective image plane is
    ...
    \item \textbf{Negligible Casimir--Polder force}:
          ...
          Lifshitz\,\cite{lifshitz1956theory}); the crossover height
          $h^*(k)$ solves $3C_3 = eE_{\rm bg}\,h^4e^{-kh}$, and for
          features of wavelength comparable to the ion height
          ($kh = 2\pi$, i.e.\ $\lambda \approx h$) the crossover is
          $h^*\approx30$\,nm --- the $100$\,nm exclusion
...
factor is identical for the topographic and charge channels.
\end{lemma}
...
\textbf{distinguishing feature of the ion probe}.  Band-pass spatial
filtering is, of course, a routine post-processing step in surface
metrology --- the ISO~16610 Gaussian profile filters~\cite{iso16610}
separate long- and short-wave profile components by design --- but for
QESPM the band-pass response is the native, continuous,
height-tunable physical transfer function of the sensor itself, with
sub-unity quality factor $Q=0.589$ (Corollary~\ref{cor:qfactor}), rather
than a filter applied to already measured data.
...
\lambda_{\rm min} \approx 1.51\,h
    \qquad \textup{(short-wavelength, 50\% MTF)},
\end{equation}
the long-wavelength cutoff being set in practice by the scan range (FOV).
...
\begin{enumerate}
    \item \textbf{Detection sensitivity} $\delta A_{\min}$ --- the smallest
          ...
          Quantum enhancement directly improves this quantity.
    ...
    \item \textbf{Height reconstruction uncertainty} $u_c(z)$ --- the
          ...
          rather than measurement noise.  Quantum enhancement does
          not reduce systematic uncertainties.

    \item \textbf{Lateral spatial resolution} --- the
          smallest surface wavelength that can be independently
          reconstructed (quantified here by the 50\% MTF short-wavelength
          cutoff $\lambda_{\min}^{\rm(MTF)}\approx1.51h$).  Governed by
          the $e^{-kh}$ exponential decay
          of the ITF, which imposes an irreducible information cutoff.
          No amount of quantum enhancement can recover spatial
...
The forward operator $\mathbf{H}$ mapping $z_s \to \Delta\omega_x$ on the
finite periodic lattice below is a finite-dimensional linear map --- hence
compact --- whose singular value decomposition (SVD) reveals the
...
On the analysed 1D cross-section, the effective rank scales as $r_{\rm eff} \propto 1/h$, and the Shannon
...
fundamental effective-rank limit} of the ion probe: on the analysed 1D
single-channel cross-section at $h = 40$\,\textmu m, only six Fourier-mode
coefficients lie above the stated rank threshold.  The information content drops by a
...
is a fundamental property of the $e^{-kh}$ physics, not an artifact
...
$\sigma_n$ as in Lemma~\ref{lem:weyl}, and the Shannon number
...
Improving lateral resolution (reducing $h$) improves the minimum
detectable height amplitude
...
by the quantum Cram\'er--Rao bound (see the reviews of Giovannetti et
al.~\cite{giovannetti2011advances} and Degen et al.~\cite{degen2017quantum}),
saturated by Ramsey interferometry on
...
$\mathcal{F}_A(k;h) \to 0$ regardless of how large
...
form: the quantum bound for mode $k$ diverges as
...
(Here $\lambda_{\min}$ is the noise-limited short-wavelength cutoff
...
the geometric resolution limit
...
\end{equation*}
with $\delta\omega$
in rad/s, reproducing the charge-free entries $1.2$\,nm ($5$\,Hz),
$0.17$\,nm ($0.7$\,Hz), $0.04$\,nm ($0.16$\,Hz) and $0.02$\,nm
($0.07$\,Hz); the charge-present entry of the first row
($320^{*}$\,nm) is not a value of Eq.~(\ref{eq:scaling_amin}) but the
charge-limited reconstruction uncertainty $u_c(z)=319$\,nm of
Table~\ref{tab:uncertainty}, quoted for contrast; the $\lambda_{\min}$
...
cutoff $\lambda_{\min}$ improves only logarithmically with
$\delta\omega$ ($18.7\to13.7$\,\textmu m over $70\times$); quantum
enhancement buys sensitivity, not spatial information.  At the same time,
\textbf{charge elimination provides a ${\sim}2.7\times10^2\times$
improvement} (the reconstruction-uncertainty floor of the first row
replaced by the $1.2$\,nm detection sensitivity of the second) --- far
...
\textbf{Source sets.}
Convergence of any regularisation scheme requires an a-priori
...
\textit{Proof sketch.}  The frequency axis is split at the passband
$B(\theta)=\{k:|H(k)|\ge\theta\}$: on the passband Parseval bounds the
difference by $\delta/\theta$, and on the complement the low-$k$ part is
bounded through the embedding $\|z\|_{L^\infty}\le C_s\|z\|_{H^s}$ and
the high-$k$ part from the source condition; balancing at
$\theta=\delta^{2/3}$ gives Eqs.~(\ref{eq:cond_stability})--
(\ref{eq:cond_stability_rate}) (full proof: Supplementary Material,
Sec.~S16).


\textbf{Convergence of the regularised solution.}
For $z_s^\dagger\in X_s(E)$ with Fourier support inside the MTF band and
noisy data of level $\delta$, the Tikhonov estimate obeys the noise--bias
decomposition with propagated noise $\delta/(2\lambda)$ and regularisation
bias bounded as in Theorem~\ref{thm:stability}; any choice
$\lambda(\delta)\to0$ with $\delta/\lambda(\delta)\to0$ converges in
$L^2$, and the balanced choice $\lambda(\delta)=c\,\delta^{4/5}$ gives the
rate $\mathcal{O}(\delta^{1/5}) +
E\,h^s\bigl(\ln(1/\delta)\bigr)^{-s}\bigl(1+o(1)\bigr)$ --- the logarithmic
term dominating as $\delta\to0$ is the quantitative signature of severe
ill-posedness.  The discrepancy principle attains this rate
($\lambda_{\rm DP}(\delta)\asymp\delta^{4/5}$, verified numerically as
$\lambda_{\rm DP}/|H|_{\max}\propto\delta^{0.86}$ over a factor-16 noise
range).  Full details: Supplementary Material (Sec.~S16).
...
the Tikhonov estimate is therefore order-optimal: no
...
$k\approx h^{-1}\ln(1/\delta)$ gives Eq.~(\ref{eq:minimax}) (full proof:
Supplementary Material, Sec.~S16).


The logarithmic rate has an important physical implication: halving the
measurement noise (e.g., from $5\,\textup{Hz}$ to $2.5\,\textup{Hz}$)
improves the reconstruction error only by a fixed additive constant in
the exponent.  Practical resolution gains therefore come primarily from
reducing $h$ --- which shifts the entire singular-value
spectrum --- rather than from reducing the noise floor.

\textbf{Alternative regularisation strategies.}
Truncated SVD is equivalent to a sharp low-pass filter at the MTF cutoff
and gives $r=0.979$ versus $0.981$ for Tikhonov on the test case;
Tikhonov is preferred for smoothness (no Gibbs ringing at the cutoff).
Iterative methods (Landweber, CGLS) offer no advantage for the present
linear, Fourier-diagonal problem but would be necessary for nonlinear
inversions.
...
\end{equation}
i.e.\ Tikhonov with the frequency-dependent regulariser
...
32\,\mu\textup{m}$, and the spread is bounded between ${\approx}25$\,\textmu m
(the noise-free kernel width, set by the PSF scale ${\sim}h$) and
${\approx}34$\,\textmu m (heavy regularisation) --- the lateral
...
$\Delta x_{\rm res} \approx 32$\,\textmu m; the spread is confined to the
band $25$--$34$\,\textmu m, i.e. the lateral resolution is $\mathcal{O}(h)$
...
higher correlation at larger $h$ --- is opposite to that observed
...
and $r$ increases with decreasing $h$, as confirmed in
...
to the 1D information content, suggesting that a practical scan can be
substantially sparser than a uniform $128$-point acquisition when the
sampling design is optimised.  The optimal sparse placement is the noise-aware D-optimal design,
...
sub-nanometre level and are not the limiting factor.
...
surface height can be identified.  No quantum enhancement can
...
contrast, and Bayesian joint inversion --- could reduce the charge contribution
by an estimated factor of 10--100, which would bring the combined uncertainty to
...
documentation: $u(\sigma) = 1.13 \times 10^{-8}\,\textup{C/m}^2$ is a
representative residual charge density after UV cleaning, adopted from
the post-UV surface-potential maps of~\cite{ziomba2025uv} (residual
patch potentials of order a few mV) through the
Dirichlet-to-Neumann map of Eq.~(\ref{eq:patch_mapping}): a representative
residual patch potential of $3.2$\,mV at the budget wavenumber
$k=2\times10^5$\,rad/m corresponds to
$\sigma = 2\varepsilon_0 k \phi_s = 1.13\times10^{-8}$\,C/m$^2$, adopted
as a rectangular distribution of half-width
$2 \times 10^{-8}\,\textup{C/m}^2$; the combined uncertainty scales
linearly with this prior. $u(h) = 100\,\textup{nm}$
...
formula from the component-wise degrees of freedom of the Supplementary
Material (Sec.~S7, Table~S3), exceed 100 for the combined uncertainty,
justifying the Gaussian coverage factor $k = 2$.
...
is $r(h, E_{\rm bg}) < 0$ (increasing electrode voltages increases both
$E_{\rm bg}$ and the calibrated $h$); its exact value is immaterial to the
budget, because even perfect anticorrelation ($r=-1$) contributes
$2\,u_h(z)\,u_E(z) = 2\times2.0\times0.10 = 0.40$\,nm$^2$ --- below
$0.001\%$ of the combined variance
$u_c^2(z) \approx (319\,\textup{nm})^2$.  All correlations are therefore
neglected, with an error below $0.001\%$ of the variance.
...
Scanning over the broadband roughness of a metallic surface compatible
with the perfect-conductor assumption, the spectrum
$S_{\Delta\omega}(k) \propto k^4 e^{-2kh} S_z(k)$ should peak at
$k = (4-\beta)/(2h)$ for a power-law roughness spectrum
$S_z(k)\propto k^{-\beta}$ (reducing to $k=2/h$ only for a white
roughness spectrum); the absence of the predicted peak at the value
appropriate to the measured roughness exponent $\beta$ rejects the
band-pass model.
...
Fitting the measured ACF to the patch-potential model by nonlinear
least squares or likelihood-based model comparison extracts $\ell_c$;
if a confidence interval consistent with $\ell_c=0$ is obtained and the
white-noise model is not disfavoured against the correlated-patch model,
the correlated-patch hypothesis is rejected.
...
lies below the 50\% MTF short-wavelength cutoff
$\lambda_{\min}\approx60$\,\textmu m.  The corresponding reconstruction and
...
propagated through the ITF, with the $5$\,Hz white-noise floor of
Sec.~\ref{sec:noise} added to the simulated ion-signal maps.  \textbf{This is a self-consistency and
...
(Figs.~\ref{fig:real_afm}d,f).  This lowest-height point remains
SNR-feasible despite the heating limit: at $h = 5$\,\textmu m the RMS
shift for $R_a = 100$\,nm reaches $\sigma_{\Delta\omega}/2\pi \approx
4.3\times10^{5}$\,Hz (Sec.~3.9), far above the heating-limited frequency
resolution of ${\sim}0.8$\,kHz (Sec.~\ref{sec:noise}).  The pixel-level
correlations are dominated by the resolvable surface features; because
the surface is spatially correlated (correlation length of order
$\ell_c\sim5$\,\textmu m, so the $20\times20$\,\textmu m field
contains only $\sim(20/\ell_c)^2\approx16$ effectively independent
patches), pixel-count standard errors are not meaningful and are not
quoted.  The monotonic improvement of $r$ with decreasing $h$ is the
model-predicted consequence of the $e^{-kh}$ filter (Sec.~\ref{sec:itf});
this benchmark is therefore a demonstration of consistency between the
reconstruction chain and the predicted height dependence, not a
significance test.  \textbf{The $h = 2$\,\textmu m data point
...
spatial-information limit of the technique under idealised (zero-heating)
conditions.
...
design.  The scaling prediction --- $|\Delta\omega_x| \propto e^{-kh}$ ---
...
--- the interval is correctly calibrated.  The coverage statement is
conditional on the stated priors: because the charge row contributes
$99.99\%$ of the variance and $u_c(z)$ is linear in $u(\sigma)$ (the
sensitivity coefficient is fixed by the physics), scaling the charge prior
by a factor $\gamma$ rescales $u_c(z)$ by approximately $\gamma$, so the
charge-controlled scenarios of Sec.~\ref{sec:uncertainty} bound the
consequence of prior uncertainty.  Full details are given in the
...
with $h$.  Throughout this work, lateral resolution is therefore not
represented by a single universal number: the ESF width, the PSF
central-lobe scale, the Rayleigh-style two-point criterion and the
50\% MTF cutoff quantify different aspects of the spatial response.
...
published reviews rather than certified specifications; the KPFM/EFM
entries follow the sources cited in Table~\ref{tab:sensitivity}.}
...
(i)~it senses the field gradient (second derivative of the
...
(ii)~it reconstructs a map through a regularised inversion of a
...
\caption{Sensitivity--standoff landscape for QESPM relative to
...
parameter space: it provides sub-nanometre vertical detection
...
(i)~the quantitative forward model and ITF that converts raw
frequency-shift data into surface height through a regularised inversion of
a severely ill-posed inverse problem;
(ii)~the metrological framework (uncertainty budget, calibration
...
the topography--charge degeneracy as the central fundamental obstacle,
...
rank could be restored only by a non-electrostatic channel
...
field sampled on a $128\times128$ grid at $1$\,s/pixel,
...
submodularity guarantee) are information-efficient under the stated
model and acquisition assumptions: a 1D demonstration reduces
...
\begin{enumerate}
    ...
    \item \textbf{The five-row regime comparison} (Table~\ref{tab:regimes})
...
dominant systematic uncertainty.  Charge mitigation could substantially
reduce the present charge-limited reconstruction uncertainty, but the
resulting performance remains to be established experimentally.
