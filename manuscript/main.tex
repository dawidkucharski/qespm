...
sample relative to a mean reference plane; the physical sensor is a single
trapped atomic ion, held at height $h$ above the surface in a
radio-frequency Paul trap, which converts the surface electrostatic
perturbation into a shift $\Delta\omega_x$ of its secular motional
frequency.  The measurement operator is characterised by the instrument
transfer function (ITF), derived from
boundary-perturbation theory on Laplace's equation,
$H_x(k;h) \propto k_x^2 e^{-kh}$ --- a
...
identical $e^{-kh}$ scaling (rank-1 joint measurement matrix) --- surface
charge is therefore not a noise source but an influence quantity
structurally confounded with the measurand.  Analytical
...
spectral shape, $\sigma_{\Delta\omega}\propto R_a$ \emph{exactly}
for any such surface --- the linear scaling is a mathematical identity of
the linear model, not a numerical coincidence.  The identity was verified
to machine precision against the FFT implementation and reproduces the
Monte Carlo result ($68.0\pm2.5$\,Hz/nm at $h=40$\,\textmu m over 30
realisations); the
...
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{1.1cm}>{\raggedright\arraybackslash}p{0.9cm}>{\raggedright\arraybackslash}p{1.0cm}>{\raggedright\arraybackslash}p{2.0cm}>{\raggedright\arraybackslash}p{1.0cm}>{\raggedright\arraybackslash}p{1.3cm}@{}}
\hline
...
Optical & geometry & optical & mm & ${\sim}0.5$\,\textmu m & ${\sim}1$\,nm & broad & diffraction-limited \\
SEM & electron contrast & electron beam & nm & ${\sim}1$\,nm & ${\sim}1$\,nm & vacuum & beam--sample interaction \\
KPFM/EFM & potential + topography & electrostatic & nm--\textmu m & nm--\textmu m & ${\sim}$mV (potential) & broad & potential--topography cross-talk \\
\textbf{QESPM} & electrostatic geometry & electrostatic & $1$--$100$\,\textmu m & ${\sim}1.5h$ (MTF) & ${\sim}10^{-2}$\,nm (det.);\newline $319$\,nm (charge-limited) & UHV--cryo native & topography--charge identifiability \\
\hline
\end{tabular}
\end{table}
...
resolution--sensitivity plane.  QESPM targets the long-standoff,
non-contact sector of the plane, where the vertical axis denotes
\emph{detection sensitivity}; the realistic topographic reconstruction
uncertainty of Table~\ref{tab:uncertainty} is shown separately.
(b) Research gap
